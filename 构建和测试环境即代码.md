...
- Real或fake的Exchange rate service。
- Real或fake的database。
- Test dirver。
...
我们期望项目的测试包含：
- unit test，单元测试直接在test dirver容器中运行。
- integration test，集成测试需要启动Exchange rate service和database容器，并在test dirver容器中运行集成测试。
- journey test，journey需要启动Exchange rate service和database容器和International transfers service，并在test dirver容器中运行journey测试。
...
prerequisites中的task会在最开始执行，所以执行顺序：`prerequisites` -> `prerequisites` -> `task`。
